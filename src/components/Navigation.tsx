...
export const Navigation = () => {
  ...
  return (
    <nav className="fixed left-4 md:left-6 top-1/2 -translate-y-1/2 z-[950] flex flex-col gap-3 pointer-events-auto">
            ...
            className={({ isActive }: { isActive: boolean }) =>
              `group relative flex items-center justify-center w-12 h-12 rounded-2xl transition-all duration-200 backdrop-blur-xl border ${
                isActive
                  ...
                  : 'bg-white/90 dark:bg-slate-900/90 border-slate-200/90 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 shadow-sm'
              }`
            }
          >
            {({ isActive }: { isActive: boolean }) => (
              <>
                ...
                />
                <span className="absolute left-full ml-3 px-3 py-1.5 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-100 rounded-xl text-xs font-semibold whitespace-nowrap opacity-0 -translate-x-3 pointer-events-none transition-all duration-200 group-hover:opacity-100 group-hover:translate-x-0 shadow-xl">
                  {link.label}
                </span>
              </>
            )}
      ...
      <div className="w-8 h-[1px] bg-slate-200 dark:bg-slate-800 my-1 mx-auto" />

      {/* Theme Quick Toggle */}
      <button
        onClick={toggleTheme}
        aria-label="Toggle Theme"
        className="group relative flex items-center justify-center w-12 h-12 rounded-2xl transition-all duration-200 backdrop-blur-xl border bg-white/90 dark:bg-slate-900/90 border-slate-200/90 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 shadow-sm cursor-pointer"
      >
        {theme === 'dark' ? (
